"use strict";
window.ipc.renderer.console(["log", "Calendar: Process is ready"]);
...
window.ipc.on.updateClockwheel((event, arg) => {
	...
	if (events.length > 0) {
		...
		let topOfClock = moment.parseZone(meta.time).startOf("day").add(1, "days");
		...
		let untilTopOfClock = moment(topOfClock).diff(moment(meta.time), "minutes");
