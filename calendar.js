window.addEventListener('DOMContentLoaded', () => {
    ...
    window.ipcRenderer.on('update-clockwheel', (event, arg) => {
        ...
        if (events.length > 0) {
            ...
            var untilTopOfClock = moment(topOfClock).diff(moment(meta.time), 'minutes');

            // Populate inner ring array with events
            events
                .filter((event) => moment(event.end).isAfter(moment(meta.time), 'minutes') && moment(event.start).isSameOrBefore(moment(meta.time).add(12, 'hours'), 'minutes') && [ 'show', 'remote', 'sports', 'prerecord', 'playlist', 'genre' ].indexOf(event.type) !== -1 && [ 'canceled', 'canceled-system', 'canceled-updated' ].indexOf(event.scheduleType) === -1)
                .map((event) => {
                    ...
                    if (moment(event.start).isSameOrBefore(moment(meta.time), 'minutes')) {
                    ...
                    } else { // Event is not yet in progress
                        let length = moment(event.end).diff(moment(event.start), 'minutes');
                        let start = (720 - untilTopOfClock) + moment(event.start).diff(moment(meta.time), 'minutes');

                        // Correct length if it goes beyond 12 hours
                        if (moment(event.end).isAfter(moment(meta.time).add(12, 'hours'), 'minutes')) {
                            var correction = moment(event.end).diff(moment(meta.time).add(12, 'hours'), 'minutes');
                            length -= correction;
                        }
        ...
        outer.map((segment) => {
            ...
            if (segment.id !== currentSegment.id) {
                ...
                clockwheelDonutData.datasets[ 0 ].backgroundColor.push(currentSegment.backgroundColor);
        ...
        inner.map((segment) => {
            ...
            if (segment.id !== currentSegment.id) {
                ...
                clockwheelDonutData.datasets[ 1 ].backgroundColor.push(currentSegment.backgroundColor);
        ...
        window.ipcRenderer.send('renderer', [ 'update-clockwheel', [ clockwheelDonutData ] ])
    });
