...
try {
    ...
    var directorReq = new WWSUreq(socket, null, 'name', '/auth/director', 'Director')
    ...
    var djsdb = new WWSUdjs(socket, noReq, directorReq);
    var Timesheets = [];
    var timesheets = [];
    var timesheetsdb = new WWSUtimesheet(socket, noReq);
    var calendar;
    var calendardb = new WWSUcalendar(socket, noReq, directorReq);
...
$(document).ready(function () {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function (info) {
            console.dir(info.event.extendedProps.event);
            calendardb.showClickedEvent(info.event.extendedProps.event, djsdb, directorsdb);
        }
    });
    calendar.render();
    ...
    $('.btn-manage-djs').click(() => {
        djsdb.showDJs(djsdb, directorsdb);
    });
    $('.btn-manage-events').click(() => {
        calendardb.showSimpleEvents(djsdb, directorsdb);
    });
});

// Database event handlers
calendardb.on('calendarUpdated', () => {
    $('#calendar').block({
        message: '<h1>Loading...</h1>',
        css: { border: '3px solid #a00' },
        timeout: 30000,
            ...
            if (body.length > 0) {
                body.map((discipline) => {
                    ...
                    if (activeDiscipline || !discipline.acknowledged) {
                        disciplineModal.title = `Disciplinary action ${activeDiscipline ? `active against you` : `was issued in the past against you`}`;
                        disciplineModal.body = `<p>On ${moment(discipline.createdAt).format('LLL')}, disciplinary action was issued against you for the following reason: ${discipline.message}.</p>
                        ...
                        disciplineModal.footer = `<button type="button" class="btn btn-outline-light"
                        id="modal-${disciplineModal.id}-discipline-acknowledge">Acknowledge</button>`;
                        disciplineModal.iziModal('open');
                    }
                })
            }
            if (docb) {
