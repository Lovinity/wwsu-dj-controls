...
class WWSUsubscriptions extends WWSUevents {
	...
	init(device = null) {
		...
		this.manager
			.get("noReq")
			.request(
				{ method: "POST", url: this.endpoints.get, data: { device: device } },
				body => {
					try {
						this.subscriptions = TAFFY();
	...
	countSubscribed(unique, calendar) {
		return this.subscriptions([
			{ type: `calendar-once`, subtype: `${unique}` },
			{ type: `calendar-all`, subtype: `${calendar}` }
		]).get().length;
	}
				...
				try {
					if (response !== "OK") {
						...
						$(document).Toasts("create", {
							...
							body:
								"Unable to subscribe you to that event at this time. Please contact the engineer.",
							autohide: true,
							delay: 10000,
						...
						$(document).Toasts("create", {
							...
							delay: 30000,
							body:
								"<p>You were successfully subscribed! You will be notified through your web browser when the event goes on the air or the time changes / is canceled.</p><p>You may stop receiving notifications if you go longer than a month without visiting the WWSU listener corner.</p>",
							icon: "fas fa-bell fa-lg"
						});
						this.subscriptions.insert({ type, subtype: `${subtype}` });
						this.emitEvent("newSubscription", [
	...
	unsubscribe(ID, event) {
		this.manager.get("noReq").request(
			{
				method: "POST",
				url: this.endpoints.unsubscribe,
				data: { device: this.device, type: `calendar-once`, subtype: ID }
			},
			response => {
				try {
					if (response !== "OK") {
						...
						$(document).Toasts("create", {
							...
							body:
								"Unable to un-subscribe you to that event at this time (calendar-once). Please contact the engineer.",
							autohide: true,
							delay: 10000,
							icon: "fas fa-skull-crossbones fa-lg"
						});
								...
								try {
									if (response !== "OK") {
										...
										$(document).Toasts("create", {
											...
											title: "Un-subscription failed",
											body:
												"Unable to un-subscribe you to that event at this time (calendar-all). Please contact the engineer.",
											autohide: true,
											delay: 10000,
											icon: "fas fa-skull-crossbones fa-lg"
										});
										...
										$(document).Toasts("create", {
											...
											delay: 15000,
											body:
												"<p>You successfully un-subscribed from that event and will no longer receive any notifications for it unless you subscribe again.</p>",
										...
										this.emitEvent("removedSubscription", [
											`${ID}`,
											`${event}`,
											this.subscriptions().get()
										]);
