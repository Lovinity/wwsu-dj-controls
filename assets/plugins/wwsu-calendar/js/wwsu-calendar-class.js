 ...
 * @requires EventEmitter event emitter for JS browsers
 * @requires $ jQuery
...
class WWSUcalendar extends CalendarDb {
    ...
    addSchedule (modal, data, cb) {
        this.doConflictCheck(modal, data, 'insert', () => {
            try {
                this.requests.director.request({ dom: `#modal-${modal.id}`, method: 'post', url: this.endpoints.addSchedule, data: data }, (response) => {
                    if (response !== 'OK') {
                        $(document).Toasts('create', {
                            class: 'bg-danger',
    ...
    editSchedule (modal, data, cb) {
        this.doConflictCheck(modal, data, 'update', () => {
            try {
                this.requests.director.request({ dom: `#modal-${modal.id}`, method: 'post', url: this.endpoints.editSchedule, data: data }, (response) => {
                    if (response !== 'OK') {
                        $(document).Toasts('create', {
                            class: 'bg-danger',
    ...
    addCalendar (modal, data, cb) {
        ...
        try {
            this.requests.director.request({ dom: `#modal-${modal.id}`, method: 'post', url: this.endpoints.add, data: data }, (response) => {
                if (response !== 'OK') {
                    $(document).Toasts('create', {
                        class: 'bg-danger',
    ...
    editCalendar (modal, data, cb) {
        ...
        this.doConflictCheck(modal, data, 'updateCalendar', () => {
            try {
                this.requests.director.request({ dom: `#modal-${modal.id}`, method: 'post', url: this.endpoints.edit, data: data }, (response) => {
                    if (response !== 'OK') {
                        $(document).Toasts('create', {
                            class: 'bg-danger',
    ...
    showClickedEvent (event) {
        ...
        if ([ 'canceled', 'canceled-system', 'canceled-changed', 'unscheduled' ].indexOf(event.scheduleType) === -1) {
            choices.push(`Occurrence: Cancel`);
        }
        if ([ 'canceled', 'canceled-system', 'canceled-changed', 'unscheduled', 'updated', 'updated-system' ].indexOf(event.scheduleType) === -1) {
                            ...
                            "click": (form, e) => {
                                ...
                                switch (value.action) {
                                    ...
                                    case `Event: Edit defaults`:
                                        var _calendar = this.calendar.find().find((_event) => _event.ID === event.calendarID);
    ...
    generateFullEventCard (event) {
      ...
      <div class="card-body box-profile">
        ...
        <li class="list-group-item text-center">
            <b>${[ 'canceled', 'canceled-system', 'canceled-changed' ].indexOf(event.scheduleType) !== -1 ? `Original Time: ` : ``}${moment(event.start).format('lll')} - ${moment(event.end).format('hh:mm A')}</b>
        </li>
