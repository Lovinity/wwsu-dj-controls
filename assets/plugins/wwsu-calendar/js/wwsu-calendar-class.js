...
// REQUIRES the following WWSUmodules: noReq (WWSUreq), directorReq (WWSUreq) (only if managing calendar), djReq (WWSUreq) (Only on DJ web panel), WWSUMeta, WWSUutil, WWSUdjs (only if editing/adding calendar events), WWSUclocks (if creating a clockwheel)
					...
					(response) => {
						if (response !== "OK") {
							...
							$(document).Toasts("create", {
								...
								body: "There was an error marking the event inactive. Please report this to the engineer.",
								autohide: true,
								delay: 10000,
								icon: "fas fa-skull-crossbones fa-lg",
							});
				...
				(response) => {
					if (response !== "OK") {
						...
						$(document).Toasts("create", {
							...
							body: "There was an error marking the event active. Please report this to the engineer.",
							autohide: true,
							delay: 10000,
							icon: "fas fa-skull-crossbones fa-lg",
						});
			...
			$(this.occurrenceActionModal.body).alpaca({
				...
				options: {
					fields: {
						...
						duration: {
							helper: `If changing the duration of this occurrence, type the new duration here (in hours; decimals permitted). The current duration is <strong>${
								event.duration / 60
							}</strong>.`,
						},
						type: {
					...
					fields: {
						...
						endDate: {
							...
							validator: function (callback) {
								let value = this.getValue();
								let startTime =
									this.getParent().childrenByPropertyId["startTime"].getValue();
