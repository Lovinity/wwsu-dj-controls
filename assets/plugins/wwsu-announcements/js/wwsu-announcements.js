...
class WWSUannouncements extends WWSUdb {
    ...
    add (data, cb) {
        try {
            this.requests.director.request({ dom: this.formModal ? `#modal-${this.formModal.id}` : undefined, method: 'post', url: this.endpoints.add, data }, (response) => {
                if (response !== 'OK') {
    ...
    edit (data, cb) {
        try {
            this.requests.director.request({ dom: this.formModal ? `#modal-${this.formModal.id}` : undefined, method: 'post', url: this.endpoints.edit, data }, (response) => {
                if (response !== 'OK') {
                    $(document).Toasts('create', {
                        class: 'bg-danger',
    ...
    remove (data, cb) {
        try {
            this.requests.director.request({ method: 'post', url: this.endpoints.remove, data }, (response) => {
                if (response !== 'OK') {
                    $(document).Toasts('create', {
                        class: 'bg-danger',
        ...
        $(this.formModal.body).alpaca({
            "schema": {
                ...
                "properties": {
                    ...
                    "type": {
                        ...
                        "required": true,
                        "title": "Type",
                        "enum": [ "djcontrols", "website-toast", "website-nowplaying", "website-chat", "website-schedule", "website-request", "website-directors", "timesheet", "display-internal", "display-internal-sticky", "display-public", "display-public-sticky" ]
                    },
                    "level": {
                        "type": "string",
                        "required": true,
            ...
            "options": {
                "fields": {
                    ...
                    "type": {
                        "optionLabels": [ "DJ Controls", "Website (Pop-up)", "Website (Now Playing Page)", "Website (Chat Page)", "Website (Schedule Page)", "Website (Request Page)", "Website (Directors Page)", "Timesheet Computer", "Internal Display Sign", "Internal Display Sign (sticky)", "Public Display Sign", "Public Display Sign (sticky)" ],
                        "helper": "Where should this announcement be displayed?"
                    },
                    "level": {
                        "optionLabels": [ "Danger (red)", "Warning (yellow)", "Info (teal)", "Success (green)", "Secondary (gray)" ],
