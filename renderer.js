			...
			if (fullMeta.scheduledEnd) {
				...
				if (
					...
					if (progress >= 0) {
						...
						$(".meta-remaining-short").html("OVER");
	...
	db.filter(record => record.status <= 4)
		...
		.map(record => {
			...
			if (
				record.name === `silence` &&
				hosts.isHost &&
				record.data.includes("Silence / very low audio detected")
			) {
				window.ipc.makeNotification([
					{
...
function updateClockwheel() {
	...
	animations.add("calendar-notifications-update", () => {
		calendar.whatShouldBePlaying(events => {
			...
			// Clear all button pulsing
			$(".btn-operation-live").removeClass("pulse-success");
			$(".btn-operation-remote").removeClass("pulse-success");
			$(".btn-operation-sports").removeClass("pulse-success");
			$(".btn-operation-sportsremote").removeClass("pulse-success");
			$(".btn-operation-automation").removeClass("pulse-danger");

			// Reset if there are no available events
			if (!events || events.length === 0) {
				notifiedShouldBePlaying = ``;
				return;
				...
				for (let i in events) {
					if (!Object.prototype.hasOwnProperty.call(events, i)) continue;

					// If the event matches what is on the air now, note that in the nowEvent variable.
					if (events[i].unique === meta.meta.calendarUnique) {
						nowEvent = events[i];
						continue;
					}
					// If this condition is met, this program is scheduled to be on the air shortly (within <2 minutes).
					else if (
				...
				// If notifyEvent is null, there are no notifications to present. Exit.
				if (!notifyEvent) return;

				// At this point, there is a broadcast on the schedule. Pulse a button as applicable.
				switch (notifyEvent.type) {
					case "show":
						if (hosts.client.lockToDJ === null)
							$(".btn-operation-live").addClass("pulse-success");
						break;
					case "remote":
						if (hosts.client.makeCalls)
							$(".btn-operation-remote").addClass("pulse-success");
						break;
					case "sports":
						if (hosts.client.lockToDJ === null)
							$(".btn-operation-sports").addClass("pulse-success");
						if (hosts.client.makeCalls)
							$(".btn-operation-sportsremote").addClass("pulse-success");
						break;
				}

				// Do not continue if this DJ is not the current host of this broadcast
				if (!hosts.isHost) return;

				// Do not continue if nothing is on the air right now
				if (
				...
				// If the current broadcast is not what should be on the air right now, pulse end broadcast button
				if (nowEvent && nowEvent.unique !== notifyEvent.unique)
					$(".btn-operation-automation").addClass("pulse-danger");
				...
				if (
					...
					window.ipc.makeNotification([
						{
							title: "Another broadcast is scheduled",
							bg: "primary",
...
function recountTodos() {
	animations.add("recount-todos", () => {
		...
		$(".nav-icon-notifications").removeClass("pulse-danger");
		$(".nav-icon-notifications").removeClass("pulse-warning");
		$(".nav-icon-notifications").removeClass("pulse-info");
		$(".nav-icon-notifications").removeClass("pulse-primary");

		if (danger > 0) {
			$(".nav-icon-notifications").addClass("pulse-danger");
		} else if (orange > 0) {
			$(".nav-icon-notifications").addClass("pulse-warning");
		} else if (warning > 0) {
			$(".nav-icon-notifications").addClass("pulse-warning");
		} else if (info > 0) {
			$(".nav-icon-notifications").addClass("pulse-info");
		} else if (primary > 0) {
			$(".nav-icon-notifications").addClass("pulse-primary");
		}
	});
}
...
// Flash taskbar on new messages

messages.on("newMessage", "renderer", () => {
	window.ipc.flashMain(true);
});
