...
try {
  ...
  ipcRenderer.on('processed-calendar', (event, e) => {
    ...
    addAnimation('calendar-clock', () => {
      ...
      newSVG = document.getElementById('clock-program-2')
      newSVG.setAttribute('transform', `rotate(${e.clockwheel.start})`)
      ...
      window.requestAnimationFrame(() => {
        ...
        if (!arg[5]) {
          ...
          if (arg[4] && arg[2] >= 0) {
            ...
            temp8 = document.querySelector(`#audio-call-icon`)
            if (temp8 !== null) { temp8.style.color = `rgb(16, 16, 16)` }
          }
        }
      })
    })
  ...
  ipcRenderer.on(`audio-file-saved`, (event, arg) => {
    hostReq.request({ method: 'POST', url: nodeURL + '/logs/add', data: { logtype: 'recorder', logsubtype: 'automation', loglevel: 'info', event: `<strong>A recording was saved.</strong><br />Path: ${arg}` } }, function (response3) {
      if (recorderDialog) { window.close() }
    })
  })
...
document.querySelector('#btn-psa15-b').onclick = function () {
  errorIfLockDJ('queue a 15 second PSA', () => {
...
document.querySelector('#btn-psa30-b').onclick = function () {
  errorIfLockDJ('queue a 30 second PSA', () => {
...
document.querySelector('#btn-golive-b').onclick = function () {
  errorIfLockDJ('start a live broadcast', () => {
    prepareLive()
  })
}
...
document.querySelector('#btn-gosports-b').onclick = function () {
  errorIfLockDJ('start a sports broadcast', () => {
    prepareSports()
  })
}
...
document.querySelector('#btn-halftime-b').onclick = function () {
  errorIfLockDJ('take an extended break', () => {
...
document.querySelector('#btn-display').onclick = function () {
  errorIfLockDJ('send a message to the display signs', () => {
    prepareDisplay()
  })
}
...
document.querySelector('#btn-requests').onclick = function () {
  errorIfLockDJ('manage track requests', () => {
    ...
    .then((devices) => {
      ...
      devices.map((device, index) => {
        if (device.kind === 'audioinput') {
          if (temp !== null) { temp.innerHTML += `<option value="${device.deviceId}">${device.label || 'Microphone ' + (index + 1)}</option>` }
          if (temp2 !== null) { temp2.innerHTML += `<option value="${device.deviceId}">${device.label || 'Microphone ' + (index + 1)}</option>` }
        } else if (device.kind === 'audiooutput') {
          if (temp3 !== null) { temp3.innerHTML += `<option value="${device.deviceId}">${device.label || 'Speaker ' + (index + 1)}</option>` }
        }
      })

      if (temp !== null) { temp.value = settings.get(`audio.input.main`) || `` }

      if (temp2 !== null) { temp2.value = window.peerDevice || `` }

      if (temp3 !== null) { temp3.value = settings.get(`audio.output.call`) || `` }
    })
  ...
  try {
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      value: {
        ...
        cHostSecret: ``,
        cStartOfSemester: moment(Config.startOfSemester).format('YYYY-MM-DDTHH:mm'),
        cLofi: Config.lofi
      },
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/displaysigns/set',
          data: {
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/meta/alt/set',
          data: {
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/meta/prefix/set',
          data: {
            automation: values.mAutomation,
            genre: values.mGenre,
            playlist: values.mPlaylist,
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/onesignal/set',
          data: {
            rest: values.oRest,
            app: values.oApp
          }
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/darksky/set',
          data: {
            api: values.oApi,
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/requests/set',
          data: {
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/songsliked/set',
          data: {
            limit: values.lLimit,
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/status/music/set-verify',
          data: {
            warn: values.sWarn,
            error: values.sError,
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/status/server/set-load1',
          data: {
            warn: values.sWarn,
            error: values.sError,
            critical: values.sCritical
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/status/server/set-load5',
          data: {
            warn: values.sWarn,
            error: values.sError,
            critical: values.sCritical
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/status/server/set-load15',
          data: {
            warn: values.sWarn,
            error: values.sError,
            critical: values.sCritical
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/status/server/set-memory',
          data: {
            warn: values.sWarn,
            error: values.sError,
            critical: values.sCritical
...
document.querySelector('#btn-options-config-breaks-clock').onclick = function () {
  try {
    ...
    if (temp !== null) { temp.innerHTML = `Clockwheel Breaks (items are minutes of each hour)` }
    var temp2 = document.querySelector(`#options-modal-config-list-items`)
    if (temp2 !== null) {
      temp2.innerHTML = ``
      for (var item in Config.breaks) {
        if (Object.prototype.hasOwnProperty.call(Config.breaks, item)) {
        ...
        $('#options-modal-config-form-form').jsonForm({
          ...
          onSubmitValid: function (values) {
            ...
            directorReq.request({
              db: Directors(),
              method: 'POST',
              url: nodeURL + '/config/breaks/set-clock',
              data: {
...
document.querySelector('#btn-options-config-breaks-live').onclick = function () {
  try {
    var temp = document.querySelector(`#options-modal-config-list-label`)
    if (temp !== null) { temp.innerHTML = `Live Show Breaks` }
    var temp2 = document.querySelector(`#options-modal-config-list-items`)
    if (temp2 !== null) {
      temp2.innerHTML = ``
      for (var item in Config.specialBreaks.live) {
        if (Object.prototype.hasOwnProperty.call(Config.specialBreaks.live, item)) {
...
document.querySelector('#btn-options-config-breaks-remote').onclick = function () {
  try {
    var temp = document.querySelector(`#options-modal-config-list-label`)
    if (temp !== null) { temp.innerHTML = `Remote Broadcast Breaks` }
    var temp2 = document.querySelector(`#options-modal-config-list-items`)
    if (temp2 !== null) {
      temp2.innerHTML = ``
      for (var item in Config.specialBreaks.remote) {
        if (Object.prototype.hasOwnProperty.call(Config.specialBreaks.remote, item)) {
          var description = `Unknown`
          switch (item) {
            case 'start':
...
document.querySelector('#btn-options-config-breaks-sports').onclick = function () {
  try {
    var temp = document.querySelector(`#options-modal-config-list-label`)
    if (temp !== null) { temp.innerHTML = `Sports Breaks` }
    var temp2 = document.querySelector(`#options-modal-config-list-items`)
    if (temp2 !== null) {
      temp2.innerHTML = ``
      for (var item in Config.specialBreaks.sports) {
        if (Object.prototype.hasOwnProperty.call(Config.specialBreaks.sports, item)) {
          var description = `Unknown`
          switch (item) {
            case 'start':
              description = `Start (Queued when a broadcast starts)`
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        ...
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/breaks/set-automation',
          data: {
            during: values.tasks
          }
        }, function (response) {
    ...
    $('#options-modal-config-form-form').jsonForm({
      ...
      onSubmitValid: function (values) {
        directorReq.request({
          db: Directors(),
          method: 'POST',
          url: nodeURL + '/config/status/breaks/set',
          data: {
...
document.querySelector('#btn-options-config-categories').onclick = function () {
  try {
    var temp = document.querySelector(`#options-modal-config-list-label`)
    if (temp !== null) { temp.innerHTML = `Track Categories` }
    var temp2 = document.querySelector(`#options-modal-config-list-items`)
    if (temp2 !== null) {
      temp2.innerHTML = ``
      for (var item in Config.categories) {
        if (Object.prototype.hasOwnProperty.call(Config.categories, item) && item !== `_doNotRemove`) {
          var endText = ``
          switch (item) {
            case 'music':
            ...
            $('#options-modal-config-form-form').jsonForm({
              ...
              onSubmitValid: function (values) {
                ...
                directorReq.request({
                  db: Directors(),
                  method: 'POST',
                  url: nodeURL + '/config/categories/set',
                  data: {
                    name: values.name,
                    config: config
                  }
...
document.querySelector('#options-announcements-add').onclick = function () {
  document.querySelector('#options-announcement-starts').value = moment().format('YYYY-MM-DDTHH:mm')
  document.querySelector('#options-announcement-expires').value = moment().add(1, 'weeks').format('YYYY-MM-DDTHH:mm')
  ...
  hostReq.request({ method: 'POST', url: nodeURL + '/planner/schedule', data: {} }, function (response) {
    if (typeof response.schedule !== `undefined`) {
      ...
      var compare = function (a, b) {
        if (a.actual === null || typeof a.actual.start === `undefined`) { return 1 }
        if (b.actual === null || typeof b.actual.start === `undefined`) { return -1 }
        if (a.actual.start < b.actual.start) { return -1 }
        if (a.actual.start > b.actual.start) { return 1 }
        return 0
      }
  ...
  try {
    ...
    if (e.target) {
      ...
      if (e.target.id.startsWith(`scheduler-remove-`)) {
        recordID = parseInt(e.target.id.replace(`scheduler-remove-`, ``))
        record = Planner({ ID: recordID }).first()
  ...
  try {
    if (e.target) {
      ...
      if (e.target.id === `modal-notifications-close`) {
        ...
        recordID = parseInt(e.target.id.replace(`notification-excuse-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
        ...
        recordID = parseInt(e.target.id.replace(`notification-unexcuse-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
        ...
        recordID = parseInt(e.target.id.replace(`notification-cancel-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
        ...
        recordID = parseInt(e.target.id.replace(`notification-absent-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
    ...
    if (e.target) {
      ...
      if (e.target.id.startsWith(`modal-underwriting-edit-`)) {
        ...
        UnderwritingsSchedules = UnderwritingsSchedules.filter((schedule, index) => {
          if (schedule.forced) {
            UnderwritingsSchedulesForced.push(schedule)
            return false
          }
          return true
        })
        UnderwritingsSchedules.forEach((v) => {
          delete v.forced
          if (typeof v.dw !== `undefined` && v.dw.length === 0) { delete v.dw }
          if (typeof v.h !== `undefined` && v.h.length === 0) { delete v.h }
        })
      ...
      if (e.target.id.startsWith(`modal-underwriting-trackinfo-`)) {
        ID = parseInt(e.target.id.replace(`modal-underwriting-trackinfo-`, ``))
        loadTrackInfo(ID)
      }
    }
  } catch (err) {
  ...
  try {
    if (e.target) {
      ...
      if (e.target.id.startsWith(`options-underwriting-schedule-edit-`)) {
        ...
        ID = parseInt(e.target.id.replace(`options-underwriting-schedule-remove-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
        ...
        ID = parseInt(e.target.id.replace(`options-underwriting-schedule-show-edit-`, ``))
        loadShow(ID)
      } else if (e.target.id.startsWith(`options-underwriting-schedule-show-remove-`)) {
        ID = parseInt(e.target.id.replace(`options-underwriting-schedule-show-remove-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
  ...
  try {
    if (e.target) {
      ...
      if (e.target.id === `modal-underwriting-schedule-f-add`) {
        ...
        if (temp !== null) { document.querySelector(`#modal-underwriting-schedule-list`).innerHTML = `` }
        var schedule = { dw: [], h: [], forced: document.querySelector(`#underwriting-schedule-forced`).checked }
        for (var i = 1; i <= 7; i++) {
          if (document.querySelector(`#underwriting-schedule-dw-${i}`).checked) { schedule.dw.push(i) }
        }
        for (var i2 = 0; i2 < 24; i2++) {
          if (document.querySelector(`#underwriting-schedule-h-${i2}`).checked) { schedule.h.push(i2) }
        }
      ...
      if (e.target.id.startsWith(`modal-underwriting-schedule-f-edit-`)) {
        index = parseInt(e.target.id.replace(`modal-underwriting-schedule-f-edit-`, ``))
        schedule = { dw: [], h: [], forced: document.querySelector(`#underwriting-schedule-forced`).checked }
        for (var i3 = 1; i3 <= 7; i3++) {
          if (document.querySelector(`#underwriting-schedule-dw-${i3}`).checked) { schedule.dw.push(i3) }
        }
  ...
  try {
    ...
    if (e.target) {
      if (e.target.id.startsWith(`dj-show-logs-ignore-`)) {
        ...
        record = parseInt(e.target.id.replace(`dj-show-logs-unignore-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
        ...
        record = parseInt(e.target.id.replace(`dj-show-logs-absent-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
        ...
        record = parseInt(e.target.id.replace(`dj-show-logs-excused-`, ``))
        iziToast.show({
          timeout: 60000,
          overlay: true,
          displayMode: 'once',
    ...
    if (e.target) {
      ...
      if (e.target.id.startsWith(`dj-xp-edit-`)) {
        ...
        DJData.XP
          ...
          .map(record => {
            document.querySelector('#options-xp-date').value = moment(record.createdAt).format('YYYY-MM-DDTHH:mm')
  ...
  try {
    ...
    if (e.target) {
      ...
      if (e.target.id.startsWith(`users-o-mute`)) {
        recipient = parseInt(e.target.id.replace(`users-o-mute-`, ``))
        prepareMute(recipient)
      }
      if (e.target.id.startsWith(`users-o-ban`)) {
        recipient = parseInt(e.target.id.replace(`users-o-ban-`, ``))
        prepareBan(recipient)
      }
      if (e.target.id.startsWith(`users-l`)) {
        recipient = parseInt(e.target.id.replace(`users-l-`, ``))
        selectRecipient(recipient)
        $('#navdrawerUsers').navdrawer('hide')
      }
      if (e.target.id.startsWith(`users-a`)) {
        recipient = parseInt(e.target.id.replace(`users-a-`, ``))
        selectRecipient(recipient)
        $('#navdrawerUsers').navdrawer('hide')
      }
      if (e.target.id.startsWith(`users-n`)) {
        recipient = parseInt(e.target.id.replace(`users-n-`, ``))
        selectRecipient(recipient)
        $('#navdrawerUsers').navdrawer('hide')
      }
      if (e.target.id.startsWith(`users-u`)) {
        recipient = parseInt(e.target.id.replace(`users-u-`, ``))
        selectRecipient(recipient)
        $('#navdrawerUsers').navdrawer('hide')
      }
    }
  ...
  try {
    if (e.target) {
      ...
      if (e.target.id.startsWith('options-announcements-edit-')) {
        ...
        document.querySelector('#options-announcement-starts').value = moment(response.starts).format('YYYY-MM-DDTHH:mm')
        document.querySelector('#options-announcement-expires').value = moment(response.expires).format('YYYY-MM-DDTHH:mm')
  ...
  try {
    if (e.target) {
      ...
      if (e.target.id.startsWith('options-discipline-edit-')) {
        discipline = Discipline({ ID: parseInt(e.target.id.replace(`options-discipline-edit-`, ``)) }).first()
  ...
  try {
    if (e.target) {
      ...
      if (e.target.id === 'options-discipline-button-add') {
        errorIfLockDJ('add a disciplinary action', () => {
    ...
    if (e.target) {
      ...
      if (e.target.offsetParent !== null && e.target.offsetParent.id !== `messages-unread` && !e.target.id.startsWith('message-n-x-')) {
        ...
        if (e.target.id.startsWith('message-n-m-')) {
          ...
          message = Messages({ ID: target }).first()
          host = (message.to === 'DJ' ? 'website' : message.from)
          selectRecipient(Recipients({ host: host }).first().ID || null)
          $('#messages-modal').iziModal('open')
        }
      }
...
function doMeta (metan) {
  try {
    ...
    if (moment(Meta.time).minutes() >= 2 && moment(Meta.time).minutes() < 5 && moment(Meta.time).diff(moment(Meta.lastID), 'minutes') >= 10 && isHost) {
      ...
      if (temp !== null) { iziToast.hide({}, temp) }
    }
...
function markRead (message = null) {
  try {
    ...
    if (message === null) {
      if (activeRecipient === null) { return null }
...
function prepareLog () {
  document.querySelector('#log-datetime').value = moment(Meta.time).format('YYYY-MM-DDTHH:mm')
...
function processRequests (data, replace = false) {
  ...
  try {
    ...
    <button type="button" class="btn btn-primary" id="request-b-${datum.ID}">Play Now</button>
          ...
          temp = document.querySelector(`#request-u-${datum.ID}`)
          temp.innerHTML = `Requested By: ${datum.username !== null && datum.username !== '' ? datum.username : `Anonymous`}`
          temp = document.querySelector(`#request-m-${datum.ID}`)
      ...
      if (DJData.XP.length > 0) {
        ...
        var compare = function (a, b) {
          try {
            ...
            if (b.type === 'remote' && a.type !== 'remote') { return 1 }
            if (moment(a.createdAt).valueOf() < moment(b.createdAt).valueOf()) { return 1 }
            if (moment(a.createdAt).valueOf() > moment(b.createdAt).valueOf()) { return -1 }
            if (a.ID > b.ID) { return -1 }
            if (b.ID > a.ID) { return 1 }
...
function loadTimesheets (date) {
  try {
    if (!moment(date).isValid()) { date = moment(Meta.time) }
...
function addNotification (group, ID, level, time, notification, name, buttons) {
  ...
  temp = document.querySelector(`#notification-group-${group}`)
  if (temp === null) {
    var tempi2 = document.querySelector(`#notification-groups`)
    if (tempi2 !== null) {
...
function loadSchedule (ID = null) {
  document.querySelector(`#underwriting-schedule-forced`).checked = false
  for (var i = 1; i <= 7; i++) {
...
function loadUnderwritingTrackInfo (trackID) {
  ...
  var temp6 = document.querySelector(`#modal-underwriting-spins`)
  temp6.innerHTML = `unknown`
  var temp7 = document.querySelector(`#modal-underwriting-track-button`)
  temp7.innerHTML = ``
  if (trackID !== 0) {
    temp7.innerHTML = `<button type="button" class="btn btn-info" id="modal-underwriting-trackinfo-${trackID}"
        title="Open up additional track info, including dates of when the track aired.">Track Info / Spin Times</button>`
    Underwritingtracks
...
function loadTrackInfo (trackID) {
  ...
  noReq.request({ method: `POST`, url: nodeURL + '/songs/get', data: { ID: trackID } }, function (response) {
    try {
...
function errorIfLockDJ (action, cb) {
  if (client.lockToDJ === null || client.lockToDJ === Meta.dj) {
    ...
    iziToast.show({
      ...
      message: `You cannot ${action} because your DJ Controls is locked to a specific DJ who is not on the air at this time.`,
      position: 'center',
      drag: false,
      closeOnClick: true
    })
